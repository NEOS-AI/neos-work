...
- 사실에 근거하지 않은 전망은 bias confidence를 낮추고 riskNotes에 명시하세요.`,
    // MCP tools keep original names (tv_* / quote_get / …). Sessions register all enabled MCP tools.
    // Workflow AgentNode still uses local tools only; chart analysis is designed for Sessions + MCP.
    allowedTools: ['web_search', 'read_file'],
    outputSchema: {
      type: 'object',

/**
 * Coding domain pack — built-in workers + block id ownership.
 */

import type { DomainWorker } from '@neos-work/shared';

export const CODING_BLOCK_IDS = [
  'code_eval',
  'file_read',
  'file_write',
  'git_diff',
...
export const CODING_WORKERS: DomainWorker[] = [
  {
    id: 'coding_reviewer',
    name: '코드 리뷰어',
    domain: 'coding',
...
OWASP Top 10 보안 취약점을 반드시 확인하세요.`,
    ...
    preferredBlockIds: ['file_read', 'git_diff'],
  },
  {
...
- 생성된 테스트 파일 경로 목록을 JSON 배열로 반환`,
    ...
    preferredBlockIds: ['file_read', 'file_write', 'test_runner'],
  },
  {
    id: 'coding_refactor',
    name: '리팩터링 에이전트',
...
리팩터링 후 변경 사항 요약을 JSON으로 반환하세요.`,
    ...
    preferredBlockIds: ['file_read', 'file_write', 'git_diff'],
  },
  {
    ...
    domain: 'coding',
    isBuiltIn: true,
    description: '요구사항을 구현하고 격리 워크스페이스에 코드를 작성합니다.',
    systemPrompt: `당신은 기능 구현 전문 AI 에이전트입니다.
주어진 요구사항을 분석하고 코드를 작성·수정하세요.
- 최소 변경으로 요구사항을 충족
- 테스트 가능한 구조 유지
- 워크스페이스 밖 파일은 수정하지 마세요
구현 후 변경 파일 목록과 요약을 JSON으로 반환하세요:
{
  "filesChanged": ["path/to/file"],
  "summary": "구현 요약",
  "notes": ["주의사항"]
}`,
    allowedTools: ['read_file', 'write_file', 'list_files', 'shell'],
    permissionProfile: 'execute',
    workspace: { kind: 'isolated' },
    defaultMode: 'solo',
    outputSchema: {
    ...
    constraints: { maxSteps: 30, timeoutMs: 360_000 },
    preferredBlockIds: ['file_read', 'file_write', 'code_eval', 'test_runner'],
  },
];
